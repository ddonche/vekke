...
export function GamePage(props: GamePageProps = {}) {
  ...
  const MOBILE_ROUTE_DOMINO_MAX = 56 // keep player route strips readable on mobile
  const MOBILE_ROUTE_SCALE = 0.72 // player routes should stay close to full size on mobile
    ...
    return (
      <div style={{ padding: "0.3rem 0.375rem 0.35rem", backgroundColor: "rgba(184,150,106,0.18)", [borderPosition === "top" ? "borderTop" : "borderBottom"]: "1px solid rgba(184,150,106,0.30)", flexShrink: 0 } as React.CSSProperties}>
        <div style={{ width: "100%", minWidth: 0, display: "grid", gridTemplateColumns: infoExpanded ? "minmax(0, 1fr) auto" : "auto minmax(0, 1fr) auto", alignItems: "stretch", columnGap: "0.375rem" }}>
          ...
          {!infoExpanded && (
            <div style={{ minWidth: 0, display: "grid", gridTemplateRows: "repeat(3, minmax(0, 1fr))", alignItems: "center", gap: "0.18rem", paddingLeft: "0.375rem", paddingRight: "0.375rem", borderRight: mobilePlayerSeparator, borderImage: "linear-gradient(180deg, transparent, #b8966a, transparent) 1" }}>
              {renderCompactTokenRow(g.reserves[side], side, "Res")}
              {renderCompactTokenRow(captiveCount, captiveTokenSide, "Cap")}
              {renderMobileActionIcons(side)}
            </div>
          )}
            ...
            {g.routes[side].slice(0, 3).map((r) => {
              ...
              return (
                <RouteIcon
                  ...
                  style={{ width: 46, alignSelf: "center", flex: "0 0 auto", minWidth: 0, aspectRatio: "7/13", cursor: isActive && !used ? "pointer" : "default", opacity: used ? 0.3 : 1 }}
                />
              )
            })}
          </div>
            ...
            >
              ...
              <div style={{ paddingTop: 6 }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 640 640" fill="#b0aa9e" style={{ flex: "0 0 auto" }}>
                      ...
                      <span style={{ color: g.player === "W" ? "#e8e4d8" : "#5de8f7" }}>
                        {g.player} {fmtClock(clocks[g.player as "W" | "B"])}
                      </span>
                    </div>
                    <div
                      style={{
              ...
              >
                ...
                <div
                  style={{
                    display: "grid",
                    gridTemplateRows: "minmax(0, 0.58fr) minmax(0, 0.42fr)",
                    gap: "0.375rem",
                    flexShrink: 0,
                ...
                >
                  ...
                  >
                    ...
                    {(() => {
                      const VOID_VISIBLE_MAX = 6
                      const wVisible = Math.min(g.void.W, VOID_VISIBLE_MAX)
                      const bVisible = Math.min(g.void.B, VOID_VISIBLE_MAX)
                      const wHidden = Math.max(0, g.void.W - wVisible)
                      const bHidden = Math.max(0, g.void.B - bVisible)
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {Array.from({ length: wVisible }).map((_, i) => (
                              <div
                                key={`vw${i}`}
                          ...
                          >
                            {Array.from({ length: bVisible }).map((_, i) => (
                              <div
                                key={`vb${i}`}
